...
parser.add_argument(
    '--musaro-path', 
    type=str, 
    default='../ROData/musaro_news/musaro_test.csv',
    help='The path to the dataset from Times New Roman'
)
parser.add_argument(
    '--output-path', 
    type=str, 
    default='../ROData/musaro_test_processed_cpy.tsv',
...
def verify_images(image_path: str, images_dir_path = '../ROData/musaro_news'):
...
df = pd.read_csv(args.musaro_path)
df_sarcasm = df[df['satiric'] == 1].copy()
df_nonsarc = df[df['satiric'] == 0].copy()

df_sarcasm['satiric'] = 'yes'
df_nonsarc['satiric'] = 'no'

# select the columns to be kept from both .csv files
df_sarcasm = df_sarcasm[
    ['satiric', 'topic', 'img_path', text_source]
]
...
df.to_csv(
    args.output_path,
    columns=['id', 'satiric', 'topic', 'image_path', 'text'],
    sep='\t', 
    index=False
)
